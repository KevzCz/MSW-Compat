...
import net.pixeldreamstudios.mswcompat.MSWCompat;
...
public class MSWCompatConfig {
    private static final Gson GSON = new GsonBuilder().setPrettyPrinting().create();
    private static MSWCompatConfig INSTANCE;
    private static final String CONFIG_FILE_NAME = "msw-compat.json";
    ...
    public static void load(Path configDir) {
        ...
        if (configFile.exists()) {
            try (FileReader reader = new FileReader(configFile)) {
                MSWCompatConfig loaded = GSON.fromJson(reader, MSWCompatConfig.class);
                if (loaded != null) {
                    INSTANCE = loaded;
                } else {
                    INSTANCE = new MSWCompatConfig();
                }
                MSWCompat.LOGGER.info("Loaded MSW-Compat configuration from {}", configFile.getAbsolutePath());
            } catch (IOException e) {
                MSWCompat.LOGGER.error("Failed to load config file, using defaults", e);
                INSTANCE = new MSWCompatConfig();
            }
        } else {
            INSTANCE = new MSWCompatConfig();
    ...
    public static void save(Path configDir) {
        ...
        try (FileWriter writer = new FileWriter(configFile)) {
            ...
            MSWCompat.LOGGER.info("Saved MSW-Compat configuration to {}", configFile.getAbsolutePath());
        } catch (IOException e) {
            MSWCompat.LOGGER.error("Failed to save config file", e);
        }
    }
